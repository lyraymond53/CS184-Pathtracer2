...
Ray Camera::generate_ray_for_thin_lens(double x, double y, double rndR, double rndTheta) const {
  ...
  Vector3D lensDir = Vector3D(0, 0, 0) - Vector3D(-xcam, -ycam, 1);
  Ray r = Ray(Vector3D(0, 0, 0), lensDir);

  Vector3D pLens = Vector3D(lensRadius*sqrt(rndR)*cos(rndTheta), lensRadius*sqrt(rndR)*sin(rndTheta), 0);

  double t = focalDistance;

  Vector3D resultantVec = r.at_time(t);
  Vector3D pfocus = resultantVec - pLens;
  pfocus.normalize();

  Vector3D posLensw = c2w * pLens;
  Vector3D pFocusw = c2w * pfocus;

  Ray camera = Ray(posLensw + pos, pFocusw);
